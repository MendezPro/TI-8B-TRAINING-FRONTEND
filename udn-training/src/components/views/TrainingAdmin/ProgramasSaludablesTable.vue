    ...
    <table>
      ...
      <tbody>
        <tr v-for="(entry, index) in filteredEntries" :key="index">
          ...
          <td>
            <button @click="editPrograma(entry.id)">
              <i class="fa fa-pencil" style="color: #e74c3c;"></i> Editar
            </button>
            <button @click="deletePrograma(entry.id)">
      ...
      async deletePrograma(id) {
        ...
        if (result.isConfirmed) {
          try {
            ...
            console.error("Error al eliminar el programa:", error.response?.data || error);
  
            Swal.fire({
              title: '¡Error!',
              text: 'Hubo un problema al eliminar el programa.',
              icon: 'error',
              confirmButtonColor: '#e74c3c'
            });
          }
